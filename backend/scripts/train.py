...
class SymptomsRequest(BaseModel):
    symptoms: List[int]
    user_id: str  # Ajout d'un identifiant utilisateur
...
def predict(req: SymptomsRequest):
    ...
    firebase_url = f"https://firestore.googleapis.com/v1/projects/VOTRE_PROJECT_ID/databases/(default)/documents/history"
    data = {
        "fields": {
            "user_id": {"stringValue": req.user_id},
            "symptoms": {"arrayValue": {"values": [{"integerValue": s} for s in req.symptoms]}},
            "predictions": {
                "arrayValue": {
                    "values": [
                        {
                            "mapValue": {
                                "fields": {
                                    "disease": {"stringValue": p.disease},
                                    "probability": {"doubleValue": p.probability}
                                }
                            }
                        }
